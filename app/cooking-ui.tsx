...
export function CookingPanel({farm,onCook}:{farm:FarmState;onCook:(id:CropId,hits:number)=>number}){
 ...
 return <div className="cook-recipes">{CROP_IDS.map(id=><button key={id} disabled={farm.crops[id]<2} onClick={()=>setSelected(id)}><LootArt kind="meal" crop={id} size={52}/><strong>{CROPS[id].meal}</strong><small>{CROPS[id].effect}</small><span><LootArt kind="crop" crop={id} size={23}/><StarBadge value={nextStars(farm,'crop:'+id)}/>{farm.crops[id]}/2</span></button>)}</div>;
}
