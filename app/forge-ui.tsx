...
import {Flame,Hammer,Droplets,Plus,Pause,Timer,Crosshair} from 'lucide-react';
...
function ForgeGame({farm,tier,onCraft,onBack}:{farm:FarmState;tier:1|2;onCraft:(tier:1|2,score:number)=>number;onBack:()=>void}){
 ...
 <div className="arcade-score"><strong><LootArt kind={tier===1?"grove-key":"hollow-key"} size={28}/>{KEY_RECIPES[tier].name}</strong><span><Timer size={16}/>{Math.ceil(shaping?55-r.time:Math.max(0,10-r.quenchTime))}s</span><button aria-label="Pause forging" title="Pause · P" disabled={!playing} onClick={a.pause}><Pause size={17}/></button></div>
 ...
 <div className="quench-key"><LootArt kind={tier===1?"grove-key":"hollow-key"} size={80}/><Droplets size={32}/></div>
 ...
 {a.mode==='done'&&<div className="arcade-cover result"><LootArt kind={tier===1?"grove-key":"hollow-key"} size={72}/><h3>{r.points>=90?'Masterwork!':r.points>=65?'Well forged':'Rough cut'}</h3><strong>{r.points}/100</strong><span role="status">{a.result===null?'Finishing...':a.result>0?<><StarBadge value={farm.lastCraft?.stars??1}/>Key forged</>:'Not enough crops'}</span><QualityOdds score={r.score} input={reward.current.input} luck={reward.current.luck}/><button className="primary" onClick={onBack}>Done</button></div>}
 </div>
