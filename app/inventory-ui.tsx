...
export function packItems(v:View):PackItem[]{
 ...
 if(farm){rows.push({key:'grove-key',name:'Grove Key',category:'Supplies',count:v.farm.keys.grove,color:'#80d7a2',description:'Opens one Whispering Grove expedition.',art:'grove-key'});rows.push({key:'hollow-key',name:'Hollow Key',category:'Supplies',count:v.farm.keys.hollow,color:'#b79cfa',description:'Opens one Bramble Hollow expedition.',art:'hollow-key'});}
...
export function Inventory({view:v,onSeed,onMeal,onReturn}:{view:View;onSeed:(id:CropId)=>void;onMeal:(id:CropId)=>void;onReturn:()=>void}){
 ...
 const hint=item?.category==='Seeds'?(farm?'Plant with E':'Bring home to plant'):item?.category==='Meals'?CROPS[item.crop!].effect:item?.category==='Crops'?'Cook at the kitchen':item?.key==='grove-key'?'Opens Whispering Grove':item?.key==='hollow-key'?'Opens Bramble Hollow':item?.key==='soil'?'Faster growth · +1 crop':'Faster crop growth';
