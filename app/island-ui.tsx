...
export function IslandPortal({farm,onEnter}:{farm:FarmState;onEnter:(tier:number,challenge:Challenge,meals:CropId[],stars:Stars)=>void}){
 ...
 return <div className="portal-board">
  ...
  <div className="portal-departure" data-feedback-anchor="portal">
   {hasKey?<div className="departure-key"><KeyRound size={19}/>{([1,2,3] as Stars[]).filter(s=>counts[s-1]>0).map(s=><button key={s} aria-label={'Use '+s+' star key'} aria-pressed={quality===s} onClick={()=>setChosen(s)}><StarBadge value={s}/><small>{counts[s-1]}</small></button>)}<small>+{(quality-1)*50}% drops</small></div>:<span className="departure-key"><KeyRound size={17}/>Visit the anvil for a key</span>}
