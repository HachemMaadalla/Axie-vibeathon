...
export default function Home(){
 ...
 return <main className={'game-shell '+(!isFarm?'in-dungeon':'')}>
  ...
  <header className="topbar quiet-topbar">
   ...
   <div className="day-chip" title={!isFarm?CHALLENGES[v.challenge??'calm'].name:undefined} aria-label={isFarm?'Day '+v.farm.day:'Expedition time'}>{isFarm?<Sun size={18}/>:<Moon size={18}/>} {isFarm?'Day '+v.farm.day:'Wave '+(v.wave??1)+((v.waveBreak??0)>0?' · '+Math.ceil(v.waveBreak!)+'s':'')}{!isFarm&&<StarBadge value={v.build.keyStars??1}/>}</div>
  ...
  <Dialog open={modal!==null} onOpenChange={value=>{if(!value)setModal(null);}}><DialogContent className={'game-dialog '+(modal==='inventory'?'inventory-dialog':['kitchen','travel','hero'].includes(modal??'')?'service-dialog':'')}>
   <DialogTitle className="dialog-title">{modal==='kitchen'?'Campfire':modal==='travel'?'Portal':modal==='hero'?(talkHero?HEROES[talkHero].name:'Axie'):modal==='garden'?'Garden':modal==='inventory'?'Inventory':modal==='build'?'Spellbook':modal==='map'?'Island journal':'How to play'}</DialogTitle>
   <DialogDescription className="sr-only">{modal==='kitchen'?'Cook with three timed stirs. Choose up to four meals at the dungeon portal.':modal==='travel'?'Seeds, fertilizer, and rich soil are waiting beyond the garden.':modal==='hero'?'Choose your companion.':modal==='garden'?'Equip seeds, a watering can, or a sickle, then press E at a bed.':modal==='build'?'Four spell slots. Four item slots. A new build every expedition.':modal==='inventory'?'Your supplies and expedition finds.':'Grow, cook, explore. Bring your next harvest home.'}</DialogDescription>
