...
import {IslandShop,CompanionTalk,IslandPortal} from './island-ui';
...
export default function Home(){
 ...
 const [v,setV]=useState<View>(initial),[started,setStarted]=useState(false),[modal,setModal]=useState<'kitchen'|'travel'|'help'|'hero'|'garden'|'inventory'|'build'|'shop'|null>(null),[muted,setMuted]=useState(true),[userPaused,setUserPaused]=useState(false);
 ...
 return <main className={'game-shell '+(!isFarm?'in-dungeon':'')}>
  ...
  {!started&&<section className="quiet-welcome panel"><span className="menu-kicker">LUNACIA</span><h1>Wildseed</h1><button className="primary" disabled={!v.ready||!!v.error} onClick={()=>{setStarted(true);game.current?.start();}}>{v.ready?'Enter Lunacia':'Loading...'}<Play size={18}/></button></section>}
  {started&&isFarm&&<>
  ...
  <Dialog open={modal!==null} onOpenChange={value=>{if(!value)setModal(null);}}><DialogContent className={'game-dialog '+(modal==='inventory'?'inventory-dialog':['kitchen','travel','shop','hero'].includes(modal??'')?'service-dialog':'')}>
   <DialogTitle className="dialog-title">{modal==='kitchen'?'Campfire':modal==='travel'?'Portal':modal==='shop'?'Shop':modal==='hero'?(talkHero?HEROES[talkHero].name:'Axie'):modal==='garden'?'Garden':modal==='inventory'?'Inventory':modal==='build'?'Spellbook':'How to play'}</DialogTitle>
   ...
   {modal==='shop'&&<IslandShop farm={v.farm} onTrade={(side,kind,id)=>game.current?.tradeItem(side,kind,id)}/>}
   {modal==='help'&&<div className="help-content"><ol><li><strong>Grow.</strong> Walk near a garden bed, choose a seed (1–3), then press E to plant, water, or harvest the highlighted bed. Nothing happens until you press E or the action button. Crops grow while you play.</li><li><strong>Look around.</strong> Drag the world to rotate the camera; scroll or pinch to zoom. V resets the view behind your Axie. WASD moves relative to the camera.</li><li><strong>Prepare.</strong> Press E at the campfire to cook. Press E at the shop to buy or sell. Talk to the other Axies to switch character.</li><li><strong>Explore.</strong> Attacks fire automatically. Move with WASD, arrow keys, or click/tap the ground. Space jumps; press it again for a double jump. Hold Shift to sprint and press Q to dash, even in the air.</li><li><strong>Build your spells.</strong> Defeat enemies for XP. Pick a spell or item at each level-up, upgrade it to level 3, and find its partner to unlock an evolution. Press B for recipes, or I to open your inventory. Equipment resets each expedition.</li><li><strong>Bring it home.</strong> Defeated creatures yield supplies. Win for the full haul, or keep half if you return early or fall.</li><li><strong>Go deeper.</strong> Walk to the portal, press E, and offer 4 Sunroot and 2 Moonberry to unlock Bramble Hollow and rare Embercorn.</li></ol><p>Garden progress saves locally in this browser. Farming pauses when this tab is hidden or a menu is open.</p><div className="credits"><strong>Made for Axie Vibeathon</strong><p>Axie and Sapidae characters, models, textures and animations belong to Sky Mavis and its licensors. Supplied via the event's Axie 3D asset pack. Terrain and rock generation use GameBlocks by Weihao Cheng (MIT). Crop designs and game systems are original prototype work.</p><a href="/licenses/axie-3d-RIGHTS.md" target="_blank" rel="noreferrer">Axie asset permission ↗</a><a href="/licenses/axie-3d-THIRD_PARTY_NOTICES.md" target="_blank" rel="noreferrer">Third-party notices ↗</a><a href="/licenses/GameBlocks-LICENSE.txt" target="_blank" rel="noreferrer">GameBlocks license</a><small>Unofficial work in progress. No wallet needed.</small></div></div>}
