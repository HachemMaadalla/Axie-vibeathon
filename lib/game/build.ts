export type WeaponId='thorn'|'petal'|'spore'|'storm'|'ember';
...
export const WEAPONS:WeaponId[]=['thorn','petal','spore','storm','ember'];
...
export const ITEMS:Record<ItemId,{name:string;kind:'spell'|'passive';color:string;levels:string[]}>={
 ...
 echo:{name:'Echo Seed',kind:'passive',color:'#c9b9ff',levels:['+1 thorn, petal, lightning target, and falling ember.','+2 thorns, petals, lightning targets, and falling embers.','+3 thorns, petals, lightning targets, and falling embers.']},
...
export const freshBuild=():Build=>({items:{thorn:1},evolved:[]});
...
export function spellStats(b:Build,id:WeaponId){
 ...
 return {...base,count:base.count+(id==='spore'?0:m.extra),area:base.area*m.area,damage:base.damage*m.damage,cooldown:base.cooldown*m.cooldown,level,evolved};
}
