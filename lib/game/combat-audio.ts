export type CombatSound='cast'|'hit'|'kill'|'hurt'|'storm'|'meteor'|'dash'|'cannon'|'slash'|'hammer'|'xp'|'pickup'|'levelup'|'evolve'|'jump'|'land';
...
export class CombatAudio{
 ...
 private chain=0;private lastPickup=-10;private _muted=true;
 get muted(){return this._muted;}
 set muted(value:boolean){this._muted=value;if(this.output&&this.context)this.output.gain.setValueAtTime(value?0:.72,this.context.currentTime);}
 enable(){
  ...
  if(!this.context){
   ...
   this.output=ctx.createGain();this.output.gain.value=.72;this.output.connect(compressor);compressor.connect(ctx.destination);
  }
