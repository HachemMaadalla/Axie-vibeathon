...
import {CropTimers} from './crop-timers';
...
export class WildseedGame {
 ...
 private collectDrop(kind:PickupKind,amount:number,point:T.Vector3,stars:Stars=1){
  if(kind==='xp'){this.xp+=amount;this.fx.burst(point,'#83f6d3',1,1.5);}
 ...
 if(this.keys.has('w')||this.keys.has('a')||this.keys.has('s')||this.keys.has('d')||this.keys.has('arrowup')||this.keys.has('arrowdown')||this.keys.has('arrowleft')||this.keys.has('arrowright')||this.target)this.followCamera.cancelPreview();
 ...
 if(this.mode==="farm")this.gardenArt?.update(this.elapsed,this.reducedMotion);else setGardenMotionTime(this.elapsed,this.reducedMotion);this.fx.update(active?realDt:0,active?this.camera:undefined);this.cartoon.render(this.scene,this.camera);this.actionFx.update(document.hidden?0:realDt,this.camera);this.cropTimers.update(this.camera,this.farm,this.started&&this.mode==='farm'&&!this.result);this.plotPrompt?.update(this.camera,this.farm,this.selected,this.held,this.seed,active&&this.mode==='farm'&&this.inReach&&!this.nearby);
