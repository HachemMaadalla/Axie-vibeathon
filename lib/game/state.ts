...
export function settleExpedition(farm:FarmState,loot:Loot,outcome:'won'|'escaped'|'lost',tier:number,quality?:QualityBag):Loot{const result={...emptyLoot(),...loot};if(outcome==='won')farm.clears++;else for(const k of Object.keys(result) as (keyof Loot)[])result[k]=Math.ceil(result[k]/2);for(const id of [...CROP_IDS,'fertilizer','soil'] as (keyof Loot)[]){const raw=quality?.[id]??[0,0];let left=result[id];for(const s of [3,2,1] as Stars[]){const amount=s===1?left:Math.min(left,raw[s-2]??0);if(amount)grantQuality(farm,id==='soil'||id==='fertilizer'?id:'seed:'+id,amount,s);left-=amount;}}farm.runs++;farm.day++;return result;}
