// Adapted from xt4d/GameBlocks (MIT). See lib/gameblocks/LICENSE and gameblocks_usage.md.
...
export function createTerrainMesh({
  ...
  centerRight = 0,
  centerForward = 0,
  includeCell = (_right, _forward) => true,
}) {
  ...
  for (let row = 0; row <= safeSegments; row += 1) {
    for (let col = 0; col <= safeSegments; col += 1) {
      ...
      const right = centerRight - halfSize + col * step;
      const forward = centerForward - halfSize + row * step;
      uvs[i*2] = right / 2.5;
      uvs[i*2+1] = forward / 2.5;
  ...
  for (let row = 0; row < safeSegments; row += 1) {
    for (let col = 0; col < safeSegments; col += 1) {
      if (!includeCell(centerRight - halfSize + (col + .5)*step, centerForward - halfSize + (row + .5)*step)) continue;
  ...
  geometry.setIndex(new THREE.BufferAttribute(indices.slice(0,index), 1));
