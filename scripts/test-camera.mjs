...
assert.ok(Math.abs(camera.position.distanceTo(focus)-18)<1e-6);
assert.equal(rig.distance,18);
rig.rotate(0,100000);assert.equal(rig.pitch,1.48);
rig.rotate(0,-100000);assert.equal(rig.pitch,.08);
...
const blocked=camera.position.distanceTo(focus);rig.update(1/60,player,[]);const recovering=camera.position.distanceTo(focus);assert.ok(recovering>blocked&&recovering<blocked+(rig.distance-blocked)*.2,'Clearing a wall must ease out without a camera pop');for(let i=0;i<120;i++)rig.update(1/60,player,[]);assert.ok(Math.abs(camera.position.distanceTo(focus)-18)<.001);
...
pointer('pointermove',250,100,0,2);assert.equal(rig.distance,18,'Pinch cannot zoom');
pointer('pointerup',100,100,0,1);pointer('pointerup',250,100,0,2);assert.equal(taps,1);
const beforeWheel=camera.position.clone();
for(const deltaY of [-10000,10000])event('wheel',{deltaY,deltaMode:0});
assert.equal(rig.distance,18);close(camera.position,beforeWheel);
