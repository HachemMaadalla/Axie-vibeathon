...
check('Farm interactions and the uncluttered combat-island center remain open after batching',()=>{
 ...
 g.makeArena();g.expandWorld(arena,'dungeon');const dungeon=new CollisionWorld();dungeon.capture(arena,(x,z)=>terrainHeight('dungeon',x,z));assert.ok(dungeon.count>=18);assert.equal(dungeon.blocked(new T.Vector3()),false,'Expedition arrival stays open');
 assert.equal(dungeon.blocked(new T.Vector3(11,terrainHeight('dungeon',11,0),0)),true,'Natural rock cover remains solid');
 assert.equal(arena.getObjectByName('garden-cottage'),undefined);assert.equal(arena.getObjectByName('portal'),undefined);assert.equal(WORLD_RADIUS.dungeon,WORLD_RADIUS.farm);
